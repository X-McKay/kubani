...
class TestResearchAnalystSkills:
    ...
    async def test_analyze_paper_generates_task_prompt(self):
        ...
        with patch('kubani.agents._base.skills_orchestrator.discover_kubani_skills') as mock_discover:
            ...
            agent.run = AsyncMock(return_value='{"research_type": "new_method", "main_claim": "test"}')

            paper = {
                "arxiv_id": "2601.12345",
                "title": "Test Paper",
                "authors": ["Author One"],
                "abstract": "This is a test abstract with enough content to pass validation. " * 5,  # >100 chars
                "categories": ["cs.AI"],
            }
    ...
    async def test_analyze_repo_generates_task_prompt(self):
        ...
        with patch('kubani.agents._base.skills_orchestrator.discover_kubani_skills') as mock_discover:
            ...
            # Mock the agent.run method
            agent.run = AsyncMock(return_value='{"category": "library", "spotlight_summary": "test"}')
    ...
    async def test_analyze_papers_batch(self):
        ...
        with patch('kubani.agents._base.skills_orchestrator.discover_kubani_skills') as mock_discover:
            ...
            papers = [
                {"arxiv_id": "2601.001", "title": "Paper 1", "authors": ["A"], "abstract": "Long abstract one " * 10},
                {"arxiv_id": "2601.002", "title": "Paper 2", "authors": ["B"], "abstract": "Long abstract two " * 10},
            ]
    ...
    async def test_analyze_repos_batch(self):
        ...
        with patch('kubani.agents._base.skills_orchestrator.discover_kubani_skills') as mock_discover:
            ...
            repos = [
                {"full_name": "a/repo1", "name": "repo1", "description": "Description one here", "stars": 100},
                {"full_name": "b/repo2", "name": "repo2", "description": "Description two here", "stars": 200},
            ]
