...
from kubani.framework.skills import find_skills_root, load_skills_from_filesystem
...
class TestFeedCollectorAgent:
    ...
    def test_skill_discovery(self):
        ...
        for skill in agent.skills:
            ...
            assert meta.get("domain") == "news", f"Skill {skill['name']} should have domain 'news'"
            assert (
                meta.get("category") == "collection"
...
class TestSkillMetadataCompliance:
    ...
    def test_all_news_skills_have_metadata(self):
        """All news skills should have metadata with domain field."""
        skills_root = find_skills_root()
        news_root = skills_root / "news" if skills_root.exists() else Path("kubani/skills/news")

        all_skills = load_skills_from_filesystem(news_root)

        for skill in all_skills:
            meta = skill.get("metadata", {})
            assert meta.get("domain") is not None, f"Skill {skill['name']} missing domain"
            assert meta.get("domain") == "news", f"Skill {skill['name']} should have domain 'news'"
