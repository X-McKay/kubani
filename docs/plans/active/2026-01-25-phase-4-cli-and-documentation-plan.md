...
This document provides a prescriptive guide for the final phase of the workflow refactor: exposing the new `agent_auto` workflow to users via the `kubani-dev` CLI and creating comprehensive documentation for the new architecture. This phase ensures the powerful new capabilities are usable, understandable, and maintainable.

## 2. Epic 7: Update CLI

**Goal:** To provide a seamless user experience for developers by integrating the new `AgentAutoWorkflow` with the `kubani-dev` command-line interface.

---

### **Task 7.1: Modify `agent draft` Command**

**Action:** Refactor the `kubani-dev agent draft` command to act as the primary entry point for starting the `AgentAutoWorkflow`.
...
def draft_agent(
    ...
    typer.echo("You can monitor its progress using the Temporal UI or `kubani-dev agent status`.")
...
**Verification:** Running `kubani-dev agent draft --name my-agent --description 
...
**Verification:** Running `kubani-dev agent status my-agent` should query the running workflow and display its current state. Running `kubani-dev agent cancel my-agent` should gracefully terminate the workflow.
...
-   The primary method for creating a new agent should now be `kubani-dev agent draft`.
...
**Action:** Write a new, hands-on tutorial for using the `kubani-dev agent` commands.
