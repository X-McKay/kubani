...
- a `Starbase OIDC` blueprint whose entries are all `state: absent`, which
  deletes the decommissioned Starbase application, provider, scope mapping,
  policy binding, and `starbase-operators` group
...
Removing an application therefore uses a reviewed forward GitOps change that
sets its binding, application, provider, scope mapping, and finally any
dedicated group to `state: absent`. Verify the discovery endpoint returns 404
before removing the key in a later cleanup revision. The Starbase
decommission (2026-09-06) is the worked example: its `starbase.yaml` key stays
in the ConfigMap as `state: absent` entries until that verification is done.
