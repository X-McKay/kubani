...
class PromoteWorkflow:
    ...
    2. Send promotion request to Discord
    3. Wait for approval reaction (✅ or ❌)
    ...
    async def run(self, input: PromoteWorkflowInput) -> PromoteWorkflowResult:
        ...
        try:
            ...
            production_skills = await workflow.execute_activity(
                load_existing_skills,
                args=[Path(input.skills_root), False],  # Exclude _development
            ...
            # Log warning if overlap but allowed
            if overlap_result.has_overlap:
                workflow.logger.warning(
                    f"Overlap detected with {overlap_result.overlapping_skills}, "
            ...
            # Get channel_id from the notification channel
            approval_result = await workflow.execute_activity(
                await_approval,
                args=[
                    input.notify_channel,  # channel_id
                    message_id,
                    None,  # discord_client injected by worker
            ...
            if approval_result.get("rejected"):
                return PromoteWorkflowResult(
                    promoted=False,
                    rejected_by=approval_result.get("reviewer"),
            ...
            if not promote_result.get("success"):
                return PromoteWorkflowResult(
                    promoted=False,
                    approved_by=approval_result.get("reviewer"),
